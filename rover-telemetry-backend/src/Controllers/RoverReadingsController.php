...
final class RoverReadingsController
{
    ...
    private function readingsLimitMode(array $rover, int $deviceId, array $query): array
    {
        ...
        return [
            ...
            'body' => [
                'device_uid' => $rover['device_uid'],
                'count' => count($rows),
                'readings' => array_map([ReadingFormatter::class, 'formatRow'], $rows),
            ],
        ];
    }
    ...
    {
        $points = array_map([ReadingFormatter::class, 'formatRow'], $rows);
        $gaps = [];
        ...
        while ($cursor <= $end) {
            ...
            if (isset($byStart[$key])) {
                ...
                $points[] = [
                    ...
                    'temperature_c' => ReadingFormatter::minAvgMax($byStart[$key], 'temp'),
                    'humidity_pct' => ReadingFormatter::minAvgMax($byStart[$key], 'hum'),
                    'gas_ppm' => ReadingFormatter::minAvgMax($byStart[$key], 'gas'),
                    'distance_cm' => ReadingFormatter::minAvgMax($byStart[$key], 'dist'),
                ];
