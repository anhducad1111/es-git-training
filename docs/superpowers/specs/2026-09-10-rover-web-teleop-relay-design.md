...
- A separate "Camera address" field, independently entered and persisted to `localStorage` from the desktop-app address field — `car_ip` (rover/relay) and `cam_ip` (camera unit) are different physical devices on the LAN in `robot-desktop-app/config.json`, so they need independent inputs. Defaults to `192.168.1.117` (this deployment's camera unit) when nothing is saved yet.
- The video panel is a plain `<img src="http://<camera_address>/640x480.mjpeg">` (the browser handles the MJPEG multipart stream natively — no polling, no canvas, no JS frame handling), plus its own 接続/切断 buttons for a manual connect/disconnect independent of the automatic gating below.
- **Gated by `allowState`, unlike an earlier draft of this section:** the camera feed is automatically shown when the desktop-app relay reports `allowed:true` (a `status` message) and automatically hidden whenever `allowState` is anything else — `not-allowed` (an explicit rejection, or a `status` message with `allowed:false`) or `disconnected` (the desktop-app WebSocket closes, errors, or the operator clicks 切断 on the desktop-app connection). This mirrors §3.4's intent — the web operator should only be holding the camera stream while they actually hold drive control — but is implemented here as best-effort client-side gating (this client does not know the true state of the camera unit's connection slot; see the coordination-gap note below unchanged from the previous draft).
