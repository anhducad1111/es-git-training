...
def _send_chat(app):
    ...
    gemini_key = app._config.get("gemini_api_key", "")
    if not gemini_key:
        app._chat_display.append('<span style="color: #ef4444;">Error:</span> No Gemini API key configured')
        return
    app._chat_worker = GeminiChat(gemini_key, text, sensor_data)
