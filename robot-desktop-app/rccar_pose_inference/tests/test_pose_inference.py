...
def test_infer_gates_out_pose_when_tilt_outside_calibration_range_and_moving():
    inference = _make_gimbal_test_inference()
    # Gimbal is moving → pose rejected
    inference._gimbal_provider = lambda: (90.0, 100.0, False)

    frame = np.zeros((480, 640, 3), dtype=np.uint8)
    result = inference.infer(frame)

    # Moving gimbal + out-of-range tilt → pose rejected, no track to coast on
    assert result.yaw_deg is None
    assert result.dist_m is None
    assert result.bbox is not None


def test_infer_returns_low_confidence_pose_when_tilt_outside_range_but_settled():
    inference = _make_gimbal_test_inference()
    # Settled gimbal, tilt far outside calibration → unrotated pose, low confidence
    inference._gimbal_provider = lambda: (90.0, 100.0, True)

    frame = np.zeros((480, 640, 3), dtype=np.uint8)
    result = inference.infer(frame)

    # Pose is returned (unrotated) with low confidence, not gated out
